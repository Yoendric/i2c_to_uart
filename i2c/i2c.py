...
def main():
  ...
  if len(devices) == 0:
    ...
    for device in devices:  
      print("Decimal address: ",device," | Hexa address: ",hex(device), "| Binary address", bin(device)[2:])
  if slave:
    print("Enviando mensaje")
    i2c.writeto(slave[0], read_measurement('\x01'))  # write the given buffer to the slave
    time.sleep(0.10)
    data=i2c.readfrom(slave[0],25)
    if Check_CRC(data):
